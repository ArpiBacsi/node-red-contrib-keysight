...
module.exports = function (RED) {
    ...
    function ConnectionNode(n) {
        ...
        node.keysightConnectionEventEmitter = new events.EventEmitter();
        ...
        function setState(newState, newStateCallback) {
            ...
            node.keysightConnectionEventEmitter.emit('state-change', {
                oldState: state,
                newState
            });
            state = newState;
        ...
        node.keysightConnect = function (callback) {
        ...
        node.keysightDisconnect = function (callback) {
        ...
        node.keysightExecuteCommand = function (command, callback) {
        ...
        node.keysightExecuteQuery = function (query, callback) {
        ...
        node.keysightEmitState = function() {
            node.keysightConnectionEventEmitter.emit('state-change', {
                newState: ""
            });
    ...
    RED.nodes.registerType("keysight-connection", ConnectionNode);
    ...
    function ConnectNode(config) {
        ...
        node.on("input", function (msg, send, done) {
            node.connection.keysightConnect();
        });
        ...
        node.connection.keysightConnectionEventEmitter.on('state-change', onConnectionStateChange);
        node.connection.keysightEmitState();

        node.on("close", function (done) {
            node.connection.keysightConnectionEventEmitter.off('state-change', onConnectionStateChange);
            done();
        });
    }
    RED.nodes.registerType("keysight-connect", ConnectNode);
    ...
    function DisconnectNode(config) {
        ...
        node.on("input", function (msg, send, done) {
            node.connection.keysightDisconnect();
        });
    }
    RED.nodes.registerType("keysight-disconnect", DisconnectNode);
    ...
    function getConnectionNode(node, msg, connection, connectionName, connectionNameType) {
        ...
        RED.nodes.eachNode(temp => {
            if (temp.name == connectionName && temp.type == "keysight-connection") {
    ...
    function CommandNode(config) {
        ...
        node.on("input", function (msg, send, done) {
            ...
            connectionNode.keysightExecuteCommand(evalParam(node, msg, node.command, node.commandType), makeCallback(msg, send, done));
        });
    }

    RED.nodes.registerType("keysight-command", CommandNode);
    ...
    function QueryNode(config) {
        ...
        node.on("input", function (msg, send, done) {
            const connectionNode = getConnectionNode(node, msg, config.connection, config.connectionName, config.connectionNameType);
            connectionNode.keysightExecuteQuery(evalParam(node, msg, node.query, node.queryType), makeCallback(msg, send, done));
        });
    }
    RED.nodes.registerType("keysight-query", QueryNode);

    //
    // keysight on-event
    //

    const keysightEventEmitter = new events.EventEmitter();
    keysightEventEmitter.setMaxListeners(1000);
    ...
    function OnEventNode(config) {
        ...
        this.on("input", function (msg) {
            if (eventName) {
                keysightEventEmitter.removeListener(eventName, handler);
            }
            eventName = evalParam(node, msg, node.eventName, node.eventNameType);
            if (eventName) {
                keysightEventEmitter.on(eventName, handler);
            }
        });
    ...
    RED.nodes.registerType("keysight-on-event", OnEventNode);
    ...
    function EmitEventNode(config) {
        ...
        this.on("input", function (msg) {
            ...
            if (eventName) {
                keysightEventEmitter.emit(eventName, msg)
            }
        });
    }
    RED.nodes.registerType("keysight-emit-event", EmitEventNode);
};
